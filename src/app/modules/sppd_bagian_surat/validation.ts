...
class BagianSuratValidation {
    async paramValidation(req: I_RequestCustom, res: Response, next: NextFunction): Promise<void> {
        if (!req?.params?.[sc.bagian_surat.primaryKey]) {
            sendErrorResponse(
                res,
                422,
                MessageDialog.__('error.missing.requiredEntry', { label: 'Bagian surat id' }),
                null
            );
        }
        else {
...
export default new BagianSuratValidation(); 
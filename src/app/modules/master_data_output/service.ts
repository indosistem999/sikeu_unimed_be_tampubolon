...
import { MasterDataOutputRepository } from './repository';
...
class MasterDataOutputService {
    private readonly repository = new MasterDataOutputRepository();
    ...
    async batchDelete(req: I_RequestCustom, res: Response): Promise<Response> {
        const payload: Record<string, any> = {
            ...
            deleted_by: req?.user?.user_id,
            list_output: req?.body?.list_output
        }
...
export default new MasterDataOutputService();
